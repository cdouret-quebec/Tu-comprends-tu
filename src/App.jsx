...
Le sujet du guide DOIT être choisi PARMI cette liste (choisis-en un différent à chaque génération, n'invente pas d'autre sujet) :
1. L'ACCUEIL EN FRANÇAIS D'ABORD : l'obligation légale (Charte de la langue française) de saluer et servir la clientèle en français en premier, même à Montréal ; le débat "Bonjour/Hi" ; ne passer à l'anglais que si le client le demande. Distingue bien ce qui est une obligation légale de ce qui est une habitude culturelle.
2. LA POLITIQUE DE RETOUR GÉNÉREUSE : les commerces québécois/nord-américains ont des politiques de retour et d'échange beaucoup plus souples qu'ailleurs (souvent 30-90 jours, sans motif exigé), ce qui peut surprendre un employé venant d'un pays où c'est plus restrictif.
3. LE SERVICE PROACTIF ET L'ACCUEIL RAPIDE : la norme nord-américaine où l'employé aborde le client presque immédiatement pour offrir de l'aide ("Bonjour, je peux vous aider ?"), ce qui peut sembler intrusif ou pressant pour un immigrant habitué à un service plus discret/à la demande.
4. LES POURBOIRES SUGGÉRÉS SUR LES TERMINAUX DE PAIEMENT : de plus en plus fréquents même en commerce de détail (pas juste en restauration), une source de malaise ou de confusion pour beaucoup de nouveaux arrivants — explique que ce n'est pas obligatoire même si le terminal le suggère.
NE traite PAS du tutoiement/vouvoiement avec la clientèle pour ce secteur — ce sujet est trop incertain et varie trop d'un établissement à l'autre pour être présenté de façon fiable.` : "";
      return `Tu es expert de la culture professionnelle québécoise dans le secteur "${s.label}" (${s.contexte}). Génère un mini-guide sur UN aspect culturel qui surprend les immigrants qui TRAVAILLENT dans CE secteur (point de vue du professionnel, pas du client/patient/élève).${sujetImpose}${santeNote}${commerceNote}
...
IMPORTANT sur la nuance : ces tendances ne s'appliquent pas à tout le monde ni partout — évite les généralisations absolues du type "les Québécois font X" ou "les Québécois pensent Y". Formule plutôt avec des nuances : "il n'est pas rare que...", "plusieurs collègues pourraient...", "dans plusieurs milieux de travail...", "certains le perçoivent comme...". Précise, quand c'est pertinent, que ça varie selon la région, la génération ou le milieu de travail, et que ce sont des tendances culturelles observées, pas des règles universelles.
...
function TrousGrammaireCard({ data, color }) {
  ...
  // Mélanger les mots si disponibles (mélange stable tant que data ne change pas)
  const mots = useMemo(() => {
    const source = data?.mots_a_utiliser || data?.["mots_à_utiliser"] || [];
    const arr = [...source];
  ...
  const idsDansTexte = [...data.texte_trous.matchAll(/\{\{(\d+)\}\}/g)].map(m => m[1]).sort();
  const idsDansTrous = data.trous.map(t => String(t.id)).sort();
  const desalignement = idsDansTexte.length !== idsDansTrous.length ||
    idsDansTexte.some((id, i) => id !== idsDansTrous[i]);
  if (desalignement) {
    return (
      <div style={{ textAlign: "center", padding: 24 }}>
        <p style={{ fontSize: 14, color: "#888" }}>⚠️ Ce contenu contient une incohérence entre le texte et les réponses (désalignement des trous). En mode Enseignante, supprime-le et génère-le à nouveau.</p>
      </div>
    );
  }
...
VÉRIFICATION OBLIGATOIRE avant de finaliser : relis chaque trou avec sa réponse insérée directement dans le texte qui l'entoure (les mots juste avant et juste après), et confirme que la phrase complète est grammaticalement correcte. Piège fréquent : si le texte fixe contient déjà un déterminant juste avant le trou (ex: "Ces ___", "les ___", "cette ___"), ne mets JAMAIS un autre déterminant comme réponse (ex: "des", "les") — la réponse doit être un adjectif ou un nom compatible avec le déterminant déjà présent, pas un second déterminant. Dans "mots_a_utiliser", n'utilise QUE des mots français réels et correctement orthographiés — jamais un mot inventé ou mal orthographié, même comme distracteur.
  ...
  return <div><h3 style={{ marginBottom: 14, fontSize: 17 }}>{data.titre}</h3><div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
    {data.expressions.map((e,i)=>(
      <div key={i} style={{ background: rb[e.registre]||"#F8F8F8", border: `1px solid ${(rc[e.registre]||color)}30`, borderRadius: 12, padding: 14 }}>
        ...
        {e.piege&&<p style={{ margin: "10px 0 0", fontSize: 13, color: "#7C2D12", background: "#FEF3E2", border: "1px solid #F0B429", borderLeft: "4px solid #D97706", borderRadius: "6px", padding: "7px 10px" }}>⚠️ {e.piege}</p>}
      </div>
    ))}
  </div></div>;
}

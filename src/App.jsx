...
function TrousGrammaireCard({ data, color }) {
  ...
  const fuiteDeRaisonnement = data.trous.some(t => /attendez|ce trou n'est pas|correction\s*:|erreur détectée/i.test(`${t.reponse || ""} ${t.explication || ""}`));
  if (fuiteDeRaisonnement) {
    return (
      <div style={{ textAlign: "center", padding: 24 }}>
        <p style={{ fontSize: 14, color: "#888" }}>⚠️ Ce contenu contient une remarque de révision qui n'aurait pas dû apparaître dans le texte final. En mode Enseignante, supprime-le et génère-le à nouveau.</p>
      </div>
    );
  }

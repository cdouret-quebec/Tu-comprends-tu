...
IMPORTANT sur la grammaire : même très oral, le français québécois reste grammaticalement cohérent — n'invente pas de formes fautives pour paraître "plus oral". Par exemple : "tout le monde ait fini" (pas "toute le monde aye"), "va falloir que quelqu'un règle ça" (pas "quelqu'un va falloir que ça règle ça"), "j'ai checké" (pas "j'ai checkté"), "on va avoir l'air de caves" (pas "on va avoir l'air de des caves"), "Quelqu'un est-tu supposé venir le ramasser?" plutôt qu'une inversion maladroite.
IMPORTANT sur le sens réel des mots : vérifie toujours le vrai sens d'une expression avant de l'utiliser, même pour donner de la couleur locale. Par exemple, "être en bédaine" signifie "être torse nu", pas "être laissé à l'abandon" (utilise plutôt "traîne encore" pour un équipement laissé de côté) ; "ostination" signifie "entêtement", ce n'est pas une interjection pour dire "on se grouille". Dans le doute, préfère une expression plus simple mais juste plutôt qu'une expression pittoresque mal utilisée ou inventée.
Pour les anglicismes de métier (foreman, dispatcher, truck, shed, GC, overtime, etc.) : ce sont des mots qu'un immigrant peut réellement entendre sur un chantier même si son français attendrait un autre terme (ex: "contremaître" pour "foreman", "entrepreneur général" pour "GC") — indique toujours le terme français standard correspondant dans "traduction_standard". Transcris "overtime" tel quel, sans doubler de lettres.
...
JSON: {"titre":string,"lieu":string,"dialogue":[{"personnage":string,"texte":string,"note_phonetique":string}],"explications":[{"expression":string,"ce_que_ca_sonne":string,"traduction_standard":string,"categorie":"secteur"|"anglicisme"|"quebecois","registre":string}],"annotations":[{"terme":string,"definition":string}]}
